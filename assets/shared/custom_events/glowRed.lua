...
function onEvent(name, value1, value2)
    if name == 'glowRed' then
        doTweenColor('Glowing', 'dad', 'ff2b2b', 0.32, 'CubeIn')
        doTweenX('xshape', 'dad.scale', 0.1, 0.6, 'QuintIn')
        doTweenY('yshape', 'dad.scale', 0.1, 0.6, 'QuintIn')
        doTweenAngle('rotate', 'dad', 380, 0.6, 'QuintIn')
        doTweenX('movex', 'dad', 750, 0.6, 'QuintIn')
        doTweenY('moveY', 'dad', 950, 0.6, 'QuintIn')
    end
end

function onTweenCompleted(tag)
    if tag == 'Glowing' then
        dyeObject('dad', 'ff2b2b')
    end
    if tag == 'movex' then
