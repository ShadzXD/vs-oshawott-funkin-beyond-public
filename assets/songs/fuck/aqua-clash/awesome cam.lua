...
local settings = {
	...
	shiftCameraPixels = 60,
	smoothCamera = false,
	smoothZoom = true,
	smoothIntensity = .50, -- too much smooth or below 0 = camera wont move lmao
}

-----------------------------------------------------------------------
-- DON'T TOUCH / UNLESS YOU KNOW CODING (dxv)
-----------------------------------------------------------------------
...
local time = 0
local os = os
function os.clock() return time end
...
function opponentNoteHit(id, dir) shiftCamNote(dadShift, dir) end

function goodNoteHit(id, dir, typ, sus) shiftCamNote(bfShift, dir) end

function noteMiss(id, dir) shiftCamNote(bfShift, dir) end

function noteMissPress(key) shiftCamNote(bfShift, key) end

local isDead = false
function onGameOverStart() isDead = true end

function onStepHit()
   if (isDead or getProperty("isDead")) then return end

   if songName == 'Deathroll' then
      if curStep == 272 then
         settings.shiftCameraOnNote = false
         --debugPrint('Shift is false')

      elseif curStep == 512 then
         settings.shiftCameraOnNote = true
         --debugPrint('Shift is true')
      end
   end
